...
**Получаем:** заголовок окна меняется на имя проекта; в папке появляется `<name>.annproj/project.json` и `class_schema.json`
...
**Получаем:** иконка сплита меняется; при экспорте файл попадает в нужную подпапку
