"""BBoxTool — drag to draw an axis-aligned bounding box."""
...
MIN_PX = 4  # minimum box size in scene pixels
...
class BBoxTool(BaseTool):
    ...
    def _commit(self, end: QPointF):
        ...
        ann = Annotation.new(
            self._class_id, AnnotationType.BBOX,
            {"x": rect.x() / iw, "y": rect.y() / ih,
             "w": rect.width() / iw, "h": rect.height() / ih},
            tool="bbox"
        )
